...
const protectCreatorSettings = (settings: Record<string, unknown>, previous?: Record<string, unknown> | null) => {
  const { accessPassword, ...safeSettings } = settings;
...
export const appRouter = router({
  ...
  creator: router({
    ...
    createQuiz: permissionProcedure("quiz.create").input(z.object({
      ...
      const createdQuiz = await db.insert(quizzes).values({ lessonId: lesson[0]!.id, topicId: topic?.id ?? null, creatorUserId: ctx.user.id, title: input.title, slug: quizSlug, summary: input.summary, coverImageUrl: input.coverImageUrl, mode: "training", accessTier: "basic", durationSeconds: input.settings.durationMinutes * 60, passingScore: 70, questionCount: input.questions.length, randomizeQuestions: input.settings.shuffleQuestions, randomizeOptions: input.settings.shuffleAnswers, visibility: input.settings.visibility, creatorSettings: protectCreatorSettings(input.settings), status, isPublished: status === "published" });
    ...
    studioAiChat: permissionProcedure("ai.quiz.generate").input(quizStudioChatInputSchema).mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        if (created.length) {
          if (!input.requestedQuestionCount) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "AI cần số lượng câu hỏi tối đa được người tạo xác nhận trước khi tạo mới." });
          if (created.length > input.requestedQuestionCount) throw new TRPCError({ code: "BAD_REQUEST", message: `AI chỉ được tạo tối đa ${input.requestedQuestionCount} câu theo xác nhận của bạn.` });
          if ((input.context?.currentQuestionCount ?? 0) + created.length - deleted.length > 50) throw new TRPCError({ code: "BAD_REQUEST", message: "Thao tác AI vượt quá giới hạn 50 câu hỏi của Quiz." });
          const quota = await assertQuotaAvailable(ctx.user.id, "aiCreditsPerMonth");
          if (quota.limit !== null && quota.used + created.length > quota.limit) throw new TRPCError({ code: "FORBIDDEN", message: `Bạn cần ${created.length} AI Credits nhưng chỉ còn ${Math.max(0, quota.limit - quota.used)} Credit.` });
          for (const _question of created) await recordAiUsage(ctx.user.id, "generate_question");
          const responsePayload = { ...result, quota: { used: quota.used + created.length, limit: quota.limit } };
          await saveQuizStudioAiHistory({ userId: ctx.user.id, kind: "chat", label: latestPrompt, payload: { input: { prompt: latestPrompt, context: input.context ?? {}, requestedQuestionCount: input.requestedQuestionCount }, result: responsePayload } });
          return responsePayload;
        }
        const responsePayload = { ...result, quota: null };
        await saveQuizStudioAiHistory({ userId: ctx.user.id, kind: "chat", label: latestPrompt, payload: { input: { prompt: latestPrompt, context: input.context ?? {}, requestedQuestionCount: input.requestedQuestionCount ?? null }, result: responsePayload } });
        return responsePayload;
