...
export function areSameStatementSelections(answerKey: Record<string, boolean>, answerPayload?: Record<string, unknown> | null) {
  const submitted = answerPayload?.statementAnswers;
...
export function shuffledForAttempt<T>(items: T[], seed: number): T[] {
  const result = [...items]; let state = Math.abs(seed) || 1;
