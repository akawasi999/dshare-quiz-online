...
import { BadgeAlert, Bell, BookOpen, ChevronDown, CircleHelp, Globe2, LayoutDashboard, LifeBuoy, LogOut, Megaphone, Menu, Moon, ShieldCheck, Sparkles, Sun, UserRound, X, type LucideIcon } from "lucide-react";
    ...
    <div className="container flex h-[68px] items-center gap-4">
      ...
      <div className="ml-auto hidden shrink-0 items-center gap-1.5 lg:flex">
        ...
        {loading ? <span className="site-header-text px-2 text-muted-foreground">Đang xác thực...</span> : user ? <div className="relative -mx-2 inline-flex h-10 items-center px-2 after:absolute after:inset-x-0 after:top-full after:h-2 after:content-['']" onMouseEnter={openAccountDropdown} onMouseLeave={scheduleAccountClose} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) closeAccountDropdown(); }}>
          <button type="button" aria-label={`Tài khoản ${user.name?.split(" ")[0] ?? "Hồ sơ"}`} aria-expanded={accountOpen} aria-controls="account-navigation" onFocus={openAccountDropdown} onClick={openAccountDropdown} className="site-header-text flex h-10 items-center gap-2 rounded-full bg-muted px-3 text-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-primary-light hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20"><UserRound size={14} />{user.name?.split(" ")[0] ?? "Hồ sơ"}<ChevronDown size={14} className={cn("site-header-nav-chevron transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]", accountOpen && "rotate-180")} /></button>
          {accountOpen ? <div id="account-navigation" role="menu" className="absolute right-2 top-[calc(100%+0.25rem)] w-[220px] origin-top-right rounded-xl border border-border bg-surface p-2 shadow-[var(--shadow-lg)] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-2 motion-safe:zoom-in-95 motion-safe:duration-200">
