...
function NavDropdown({ item, open, onOpen, onClose, onToggle, onNavigate }: { item: NavItem; open: boolean; onOpen: () => void; onClose: () => void; onToggle: () => void; onNavigate: () => void }) {
  ...
  return <div className="relative" onMouseEnter={onOpen} onMouseLeave={onClose}>
    <button type="button" onFocus={onOpen} onClick={onToggle} aria-expanded={open} aria-controls={menuId} className={cn("group flex min-h-10 items-center gap-1 rounded-md px-0 text-sm font-medium leading-5 tracking-normal transition-[color,background-color,transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20", isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}><span>{item.label}</span><ChevronDown size={14} className={cn("transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]", open && "rotate-180")} /></button>
    {open ? <div id={menuId} role="menu" className={cn("absolute top-[calc(100%+8px)] origin-top-left border border-border bg-surface p-2 shadow-[var(--shadow-lg)] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-2 motion-safe:zoom-in-95 motion-safe:duration-200", isSupport ? "right-0 w-[230px] rounded-xl" : "left-0 w-[300px] rounded-[var(--radius-lg-token)]")}>
      ...
      {item.items?.map(child => {
        ...
        return <Link key={child.label} href={child.href} role="menuitem" onClick={onNavigate} className={cn("group/menu flex rounded-[var(--radius-sm-token)] transition-[transform,background-color,color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:translate-x-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", isSupport ? "items-center gap-3 px-3 py-2.5 hover:bg-muted" : "items-start gap-3 px-3 py-3 hover:bg-primary-light")}>
          ...
          <span className="min-w-0"><span className={cn("block text-xs font-bold transition-colors", isSupport ? "text-foreground group-hover/menu:text-primary" : "text-foreground")}>{child.label}</span>{child.description ? <span className="mt-1 block text-[11px] leading-4 text-text-secondary">{child.description}</span> : null}</span>
        </Link>;
      })}
      {!isSupport ? <Link href="/kham-pha" role="menuitem" onClick={onNavigate} className="mt-1 flex items-center justify-between rounded-[var(--radius-sm-token)] px-3 py-2 text-[11px] font-bold text-primary transition-colors hover:bg-primary-light focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Xem tất cả chủ đề <ChevronDown size={14} className="-rotate-90" /></Link> : null}
    </div> : null}
  </div>;
}
...
export default function SiteHeader({ variant = "light" }: { variant?: "light" | "dark" }) {
  ...
  return <header className="relative z-40 border-b border-border bg-surface text-foreground" data-theme-variant={variant}>
    <div className="container flex h-[68px] items-center gap-4">
      ...
      <nav className="hidden min-w-0 flex-1 items-center gap-5 xl:gap-8 lg:flex" aria-label="Điều hướng chính">
        {navigation.map(item => item.href ? <Link key={item.label} href={item.href} className={cn("min-h-10 rounded-md px-0 py-2 text-sm font-medium leading-5 tracking-normal transition-[color,transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20", location === item.href ? "text-primary" : "text-muted-foreground hover:text-foreground")}>{item.label}</Link> : <NavDropdown key={item.label} item={item} open={openMenu === item.label} onOpen={() => setOpenMenu(item.label)} onClose={() => setOpenMenu(current => current === item.label ? null : current)} onToggle={() => setOpenMenu(item.label)} onNavigate={() => setOpenMenu(null)} />)}
      </nav>
      <div className="ml-auto hidden shrink-0 items-center gap-1.5 lg:flex">
        ...
        <button type="button" aria-label="Ngôn ngữ hiển thị: Tiếng Việt" className="flex h-9 items-center gap-1 rounded-md px-2 text-sm font-medium leading-5 text-muted-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20"><Globe2 size={15} />vi</button>
        {loading ? <span className="px-2 text-sm font-medium leading-5 text-muted-foreground">Đang xác thực...</span> : user ? <><Link href="/ho-so" className="flex min-h-10 items-center gap-2 rounded-full bg-muted px-3 text-sm font-medium leading-5 text-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-primary-light hover:text-primary"><UserRound size={14} />{user.name?.split(" ")[0] ?? "Hồ sơ"}</Link><button type="button" onClick={() => logout()} className="min-h-10 rounded-full px-3 text-sm font-medium leading-5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Đăng xuất</button></> : <><Button size="sm" onClick={() => startLogin()} className="h-9 rounded-md px-4 text-sm font-medium leading-5 shadow-none"><Sparkles size={14} />Bắt đầu</Button><button type="button" onClick={() => startLogin()} className="min-h-10 rounded-md px-3 text-sm font-medium leading-5 text-muted-foreground transition-colors hover:bg-muted hover:text-primary">Đăng nhập</button></>}
      </div>
