...
function NavDropdown({ item, open, onOpen, onClose, onToggle, onNavigate }: { item: NavItem; open: boolean; onOpen: () => void; onClose: () => void; onToggle: () => void; onNavigate: () => void }) {
  ...
  return <div className="relative" onMouseEnter={onOpen} onMouseLeave={onClose}>
    <button type="button" onFocus={onOpen} onClick={onToggle} aria-expanded={open} aria-controls={menuId} className={cn("group flex min-h-10 items-center gap-1 rounded-md px-2 text-[13px] font-semibold leading-none tracking-[-.01em] transition-[color,background-color,transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20", isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}><span>{item.label}</span><ChevronDown size={14} className={cn("transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]", open && "rotate-180")} /></button>
  ...
  return <header className="relative z-40 border-b border-border bg-surface text-foreground" data-theme-variant={variant}>
    <div className="container flex h-[68px] items-center gap-4">
      ...
      <div className="ml-auto hidden shrink-0 items-center gap-1.5 lg:flex">
        ...
        <button type="button" aria-label="Ngôn ngữ hiển thị: Tiếng Việt" className="flex h-9 items-center gap-1 rounded-md px-2 text-[13px] font-semibold leading-none tracking-[-.01em] text-muted-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20"><Globe2 size={15} />vi</button>
        {loading ? <span className="px-2 text-[13px] font-semibold leading-none tracking-[-.01em] text-muted-foreground">Đang xác thực...</span> : user ? <><Link href="/ho-so" className="flex min-h-10 items-center gap-2 rounded-full bg-muted px-3 text-[13px] font-semibold leading-none tracking-[-.01em] text-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-primary-light hover:text-primary"><UserRound size={14} />{user.name?.split(" ")[0] ?? "Hồ sơ"}</Link><button type="button" onClick={() => logout()} className="min-h-10 rounded-full px-3 text-[13px] font-semibold leading-none tracking-[-.01em] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Đăng xuất</button></> : <><Button size="sm" onClick={() => startLogin()} className="h-9 rounded-md px-4 text-[13px] font-semibold leading-none tracking-[-.01em] shadow-none"><Sparkles size={14} />Bắt đầu</Button><button type="button" onClick={() => startLogin()} className="min-h-10 rounded-md px-3 text-[13px] font-semibold leading-none tracking-[-.01em] text-muted-foreground transition-colors hover:bg-muted hover:text-primary">Đăng nhập</button></>}
      </div>
