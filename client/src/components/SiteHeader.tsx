...
function NavDropdown({ item, open, onOpen, onClose, onToggle, onNavigate }: { item: NavItem; open: boolean; onOpen: () => void; onClose: () => void; onToggle: () => void; onNavigate: () => void }) {
  ...
  return <div className="relative" onMouseEnter={onOpen} onMouseLeave={onClose}>
    <button type="button" onFocus={onOpen} onClick={onToggle} aria-expanded={open} aria-controls={menuId} className={cn("group flex min-h-10 items-center gap-1 rounded-md px-1.5 text-[13px] font-semibold transition-[color,background-color,transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20", isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}><span>{item.label}</span><ChevronDown size={14} className={cn("transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)]", open && "rotate-180")} /></button>
