  ...
  return <header className="relative z-40 border-b border-border bg-surface text-foreground" data-theme-variant={variant}>
    <div className="container flex h-[68px] items-center gap-4">
      ...
      <div className="ml-auto hidden shrink-0 items-center gap-1.5 lg:flex">
        ...
        {loading ? <span className="site-header-text px-2 text-muted-foreground">Đang xác thực...</span> : user ? <><Link href="/ho-so" className="site-header-text flex min-h-10 items-center gap-2 rounded-full bg-muted px-3 text-foreground transition-[color,background-color,transform] duration-200 hover:-translate-y-px hover:bg-primary-light hover:text-primary"><UserRound size={14} />{user.name?.split(" ")[0] ?? "Hồ sơ"}</Link><button type="button" onClick={() => logout()} className="site-header-text min-h-10 rounded-full px-3 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Đăng xuất</button></> : <><Button size="sm" onClick={() => startLogin()} className="site-header-text h-9 rounded-md px-4 shadow-none"><Sparkles size={14} />Bắt đầu</Button><button type="button" onClick={() => startLogin()} className="site-header-text min-h-10 rounded-md px-3 text-muted-foreground transition-colors hover:bg-muted hover:text-primary">Đăng nhập</button></>}
      </div>
