...
export default function SiteHeader({ variant = "light" }: { variant?: "light" | "dark" }) {
  ...
  return (
    <header className={cn("relative z-40 border-b", isDark ? "border-white/10 text-white" : "border-[#2a4365]/10 text-[#2a4365]") }>
      <div className="container flex h-[76px] items-center justify-between gap-5">
        <Link href="/" className="group flex items-center gap-3" aria-label="Dshare Quiz Online">
          <span className={cn("grid h-10 w-10 place-items-center rounded-[14px] shadow-sm transition-transform duration-200 group-hover:-rotate-3", isDark ? "bg-[#4299e1] text-white" : "bg-[#2a4365] text-white") }>
            <BookOpen size={19} strokeWidth={2.3} />
          </span>
          <span className="leading-none">
            <span className="block font-serif text-[19px] font-semibold tracking-[-0.04em]">dshare</span>
            <span className={cn("mt-1 block text-[9px] font-bold uppercase tracking-[0.22em]", isDark ? "text-[#4299e1]" : "text-[#3182ce]")}>quiz online</span>
          </span>
        </Link>

        <nav className="hidden items-center gap-7 lg:flex" aria-label="Điều hướng chính">
          {links.map(link => (
            <Link key={link.href} href={link.href} className={cn("text-[13px] font-medium transition-colors hover:text-[#3182ce]", location === link.href && "text-[#3182ce]")}>{link.label}</Link>
          ))}
          <span className={cn("h-4 w-px", isDark ? "bg-white/20" : "bg-[#13293d]/15")} />
          <a href="#ve-dshare" className="text-[13px] font-medium transition-colors hover:text-[#3182ce]">Về Dshare</a>
        </nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {loading ? <span className="text-xs opacity-70">Đang xác thực...</span> : user ? (
            ...
            <>
              <button onClick={() => startLogin()} className="px-3 py-2 text-xs font-semibold transition-colors hover:text-[#3182ce]">Đăng nhập</button>
              <Button onClick={() => startLogin()} className="h-10 rounded-full bg-[#3182ce] px-5 text-xs font-bold text-white shadow-[0_10px_25px_rgba(49,130,206,.2)] hover:bg-[#2a4365]">Bắt đầu học</Button>
            </>
          )}
        </div>
      ...
      {menuOpen && <div className={cn("absolute left-0 top-[76px] w-full border-b px-5 py-5 shadow-xl lg:hidden", isDark ? "border-white/10 bg-[#102d43]" : "border-[#13293d]/8 bg-[#fffdf8]") }>
        <div className="mx-auto flex max-w-md flex-col gap-1">
          ...
          <div className="mt-3 flex gap-2 px-2">{user ? <Button asChild className="w-full rounded-full"><Link href="/ho-so"><UserRound size={15} /> Hồ sơ học tập</Link></Button> : <Button className="w-full rounded-full bg-[#3182ce]" onClick={() => startLogin()}><Sparkles size={15} /> Đăng nhập để học</Button>}</div>
        </div>
      </div>}
    </header>
  );
