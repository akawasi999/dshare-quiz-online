...
const navigation: NavItem[] = [
  ...
  { label: "Khám phá", href: ROUTES.explore },
  { label: "Bảng giá", href: ROUTES.pricing },
  { label: "Blog", href: ROUTES.explore },
  { label: "Hỗ trợ khách hàng", kind: "support", items: [
    { label: "Câu hỏi thường gặp", href: ROUTES.support, icon: CircleHelp, iconClassName: "bg-sky-100 text-sky-500" },
    ...
    { label: "Thông báo", href: ROUTES.dashboard, icon: Megaphone, iconClassName: "bg-rose-100 text-rose-500" },
  ] },
];
...
function AccountNotificationBell({ data, onRead, onReadAll, compact = false }: { data: { items: any[]; unreadCount: number } | undefined; onRead: (id: number) => void; onReadAll: () => void; compact?: boolean }) {
  ...
  useEffect(() => {
    if (!open) return;
    const dismiss = (event: MouseEvent | TouchEvent) => { if (notificationRef.current && !notificationRef.current.contains(event.target as Node)) setOpen(false); };
    const dismissEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setOpen(false); };
    document.addEventListener("mousedown", dismiss);
    document.addEventListener("touchstart", dismiss);
    document.addEventListener("keydown", dismissEscape);
    return () => { document.removeEventListener("mousedown", dismiss); document.removeEventListener("touchstart", dismiss); document.removeEventListener("keydown", dismissEscape); };
  }, [open]);
  return <div ref={notificationRef} className={cn("relative", compact ? "" : "")}><button type="button" aria-label={unreadCount ? `Mở thông báo, ${unreadCount} chưa đọc` : "Mở thông báo"} aria-expanded={open} aria-controls="account-notifications" onClick={() => setOpen(value => !value)} className="relative grid size-9 place-items-center rounded-full text-muted-foreground transition-[color,background-color,transform] duration-200 hover:scale-105 hover:bg-muted hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20"><Bell size={17} />{unreadCount ? <span aria-label={`${unreadCount} thông báo chưa đọc`} className="absolute -right-0.5 -top-0.5 grid min-w-4 place-items-center rounded-full bg-danger px-1 text-[9px] font-bold leading-4 text-white">{unreadCount > 9 ? "9+" : unreadCount}</span> : null}</button>{open ? <section id="account-notifications" role="menu" aria-label="Lịch sử thông báo" className="account-dropdown absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(360px,calc(100vw-1.5rem))] overflow-hidden rounded-xl border border-border bg-surface shadow-[var(--shadow-lg)] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-2 motion-safe:zoom-in-95 motion-safe:duration-200"><div className="flex items-center justify-between border-b border-border-light px-4 py-3"><div><p className="text-sm font-bold text-foreground">Thông báo</p><p className="text-[11px] text-text-secondary">{unreadCount ? `${unreadCount} mục chưa đọc` : "Đã đọc tất cả"}</p></div>{unreadCount ? <button type="button" onClick={() => onReadAll()} className="text-xs font-semibold text-primary hover:underline">Đánh dấu đã đọc</button> : null}</div><div className="max-h-[min(420px,calc(100vh-132px))] overflow-y-auto p-1.5">{items.length ? items.map(item => <Link key={item.id} href={item.href || ROUTES.account} role="menuitem" onClick={() => { if (!item.isRead) onRead(item.id); setOpen(false); }} className={cn("block rounded-lg px-3 py-3 transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", !item.isRead && "bg-primary-light/50")}><div className="flex items-start gap-2.5"><span className={cn("mt-1.5 size-2 shrink-0 rounded-full", item.isRead ? "bg-transparent" : item.type === "quiz_rejected" ? "bg-danger" : "bg-primary")} /><span className="min-w-0 flex-1"><span className="block text-xs font-bold text-foreground">{item.title}</span><span className="mt-1 block text-xs leading-5 text-text-secondary">{item.body}</span><span className="mt-1.5 block text-[10px] text-text-muted">{new Date(item.createdAt).toLocaleString("vi-VN", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</span></span></div></Link>) : <div className="px-4 py-9 text-center"><Bell className="mx-auto text-text-muted" size={22} /><p className="mt-2 text-sm font-semibold text-foreground">Chưa có thông báo</p><p className="mt-1 text-xs leading-5 text-text-secondary">Cập nhật quyền, gói và kết quả duyệt Quiz sẽ xuất hiện tại đây.</p></div>}</div></section> : null}</div>;
}
...
export default function SiteHeader({ variant = "light" }: { variant?: "light" | "dark" }) {
  ...
  const configuredNavigation: NavItem[] = (managedNavigation.data?.length ? managedNavigation.data.map(item => item.label === "Khám phá" ? { label: item.label, href: ROUTES.explore } : { label: item.label, href: item.url }) : navigation);
  ...
  const openAuthDialog = (mode: "login" | "register") => { openAuth({ mode, returnTo: `${location}${window.location.search}` }); };
  ...
  return <header className="site-header relative z-40 border-b border-border bg-surface text-foreground" data-theme-variant={variant}>
    <div className="container flex h-[var(--site-header-height,68px)] items-center gap-4">
      <Link href={ROUTES.home} className="group flex shrink-0 items-center" aria-label="Dshare Quiz Online"><BrandLogo src={configuredLogo || undefined} monochrome={isDarkLogo && !configuredLogo} className="h-8 max-w-[122px] transition-transform duration-200 group-hover:scale-[1.02] sm:h-9 sm:max-w-[142px]" /></Link>
