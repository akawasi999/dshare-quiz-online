...
import { UsernameWithPlan } from "./SubscriptionPlanBadge";
...
const navigationSections: NavigationSection[] = [
  ...
  { label: "Users", items: [
    { icon: Users, label: "Người dùng", path: ROUTES.adminUsers, description: "Tài khoản người dùng và trạng thái tài khoản" },
...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  const accountSummary = trpc.learner.summary.useQuery(undefined, { enabled: Boolean(user) });
...
function AdminMenu({ userName, email, tier, onLogout, onAccount, collapsed = false, compact = false }: { userName?: string | null; email?: string | null; tier?: string | null; onLogout: () => void; onAccount: () => void; collapsed?: boolean; compact?: boolean }) {
  return <DropdownMenu><DropdownMenuTrigger asChild><button type="button" className={`flex items-center gap-3 rounded-[var(--radius-md-token)] text-left transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20 ${compact ? "size-10 justify-center" : "w-full p-1.5"}`} aria-label="Mở menu tài khoản quản trị"><Avatar className="size-8 shrink-0 border border-border"><AvatarFallback className="bg-primary-light text-xs font-bold text-primary">{userName?.charAt(0).toUpperCase() ?? "A"}</AvatarFallback></Avatar>{!compact && !collapsed ? <div className="min-w-0"><UsernameWithPlan name={userName || "Quản trị viên"} tier={tier} nameClassName="max-w-[150px] text-xs font-semibold" /><p className="mt-0.5 truncate text-[10px] text-text-secondary">{email || "Admin Control Center"}</p></div> : null}</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52"><DropdownMenuItem onClick={onAccount} className="cursor-pointer"><UserRound className="mr-2 size-4" />Hồ sơ tài khoản</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={onLogout} className="cursor-pointer text-danger focus:text-danger"><LogOut className="mr-2 size-4" />Đăng xuất</DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}
