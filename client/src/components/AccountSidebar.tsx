...
export default function AccountSidebar() {
  ...
  return <aside className="sticky top-0 hidden h-[calc(100vh-76px)] w-64 shrink-0 border-r border-[#172554]/10 bg-white/70 p-4 backdrop-blur lg:flex lg:flex-col"><div className="rounded-2xl bg-[#eef4ff] p-4"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#2563eb] font-serif text-lg font-bold text-white">{user?.name?.slice(0, 1).toUpperCase() ?? "D"}</div><p className="mt-3 truncate text-sm font-bold text-[#172554]">{user?.name ?? "Tài khoản Dshare"}</p><p className="mt-1 text-[11px] text-[#617786]">Không gian học tập cá nhân</p></div><nav className="mt-5 space-y-1" aria-label="Điều hướng tài khoản">{links.map(([Icon, label, href]) => { const active = location === href.split("#")[0]; return <button key={label} type="button" onClick={() => href.startsWith("mailto:") ? window.location.assign(href) : setLocation(href)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold transition ${active ? "bg-[#172554] text-white" : "text-[#617786] hover:bg-[#eef4ff] hover:text-[#172554]"}`}><Icon size={16} />{label}</button>; })}</nav><button type="button" onClick={logout} className="mt-auto flex w-full items-center gap-3 rounded-xl border border-[#172554]/10 px-3 py-2.5 text-left text-xs font-bold text-[#aa5146] hover:bg-[#fff7f5]"><LogOut size={16} />Đăng xuất</button></aside>;
}
