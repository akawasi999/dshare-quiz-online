...
import { Award, CreditCard, GraduationCap, HandHeart, History, LayoutDashboard, LogOut, Medal, PanelLeftClose, PanelLeftOpen, PlusCircle, Sparkles, Target, Trophy, UserRound, UsersRound } from "lucide-react";
...
export default function AccountSidebar() {
  ...
  return <aside aria-label="Thanh điều hướng tài khoản" className={`sticky top-[76px] hidden h-[calc(100dvh-76px)] shrink-0 self-start overflow-hidden border-r border-border bg-surface/90 p-3 backdrop-blur lg:flex lg:flex-col ${collapsed ? "w-[76px]" : "w-64"} transition-[width] duration-200 [transition-timing-function:var(--ease-out)] motion-reduce:transition-none`}>
    <div className={`flex shrink-0 items-center border-b border-border-light pb-3 ${collapsed ? "justify-center" : "justify-between"}`}>
      <p className={`overflow-hidden text-[10px] font-extrabold uppercase tracking-[.18em] text-text-muted transition-[max-width,opacity] duration-200 motion-reduce:transition-none ${collapsed ? "max-w-0 opacity-0" : "max-w-40 opacity-100"}`}>Điều hướng</p>
    ...
    <nav className="mt-3 min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain pr-1" aria-label="Điều hướng tài khoản">{navigationGroups.map(group => <section key={group.label}><p className={`mb-1 overflow-hidden px-3 text-[10px] font-extrabold uppercase tracking-[.15em] text-text-muted transition-[max-width,opacity] duration-200 motion-reduce:transition-none ${collapsed ? "max-h-0 max-w-0 py-0 opacity-0" : "max-h-6 max-w-40 py-1 opacity-100"}`}>{group.label}</p><div className="space-y-1">{group.items.map(([Icon, label, href]) => { const active = location === href.split("#")[0]; const navigating = navigatingTo === href; return <button key={label} type="button" onClick={() => navigate(href)} title={collapsed ? label : undefined} aria-label={collapsed ? label : undefined} className={`group relative flex min-h-11 w-full items-center rounded-[var(--radius-md-token)] text-left text-xs font-semibold transition-[background-color,color,transform] duration-200 [transition-timing-function:var(--ease-out)] hover:-translate-y-px active:scale-[.98] motion-reduce:transition-none ${collapsed ? "justify-center px-0" : "gap-3 px-3"} ${active ? "bg-primary-light text-primary shadow-[inset_3px_0_0_var(--primary)]" : "text-muted-foreground hover:bg-muted hover:text-foreground"} ${navigating ? "scale-[.98] opacity-75" : ""}`} aria-current={active ? "page" : undefined}><Icon size={16} className={`shrink-0 transition-transform duration-200 group-hover:scale-110 motion-reduce:transition-none ${active ? "text-primary" : ""}`} /><span className={`overflow-hidden whitespace-nowrap transition-[max-width,opacity,transform] duration-200 [transition-timing-function:var(--ease-out)] motion-reduce:transition-none ${collapsed ? "max-w-0 -translate-x-1 opacity-0" : "max-w-44 translate-x-0 opacity-100"}`}>{label}</span></button>; })}</div></section>)}</nav>
