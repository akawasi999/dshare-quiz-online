...
export default function QuizCard({ quiz, compact = false }: { quiz: ShowcaseQuiz; compact?: boolean }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[var(--radius-xl-token)] border border-border bg-surface shadow-[var(--shadow-sm)] transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-[var(--shadow-md)]">
      {quiz.coverImage ? <div className="relative h-32 overflow-hidden bg-muted"><img src={quiz.coverImage} alt={`Ảnh bìa ${quiz.title}`} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-foreground/60 via-transparent to-transparent" />{isNew || quiz.isTrending ? <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">{newBadge}{trendingBadge}</div> : null}<div className="absolute bottom-3 left-4 right-4 flex items-end justify-between gap-3"><p className="truncate text-[10px] font-bold uppercase tracking-[.16em] text-white">{quiz.category} · {quiz.lesson}</p>{modeBadge}</div></div> : null}
      <div className="p-5">
        ...
        <h3 className="text-xl font-bold leading-tight tracking-[-.025em] text-foreground">{quiz.title}</h3>
