...
import { ArrowUpRight, Award, Clock3, Flame, LockKeyhole, Sparkles, UsersRound } from "lucide-react";
...
export default function QuizCard({ quiz, compact = false }: { quiz: ShowcaseQuiz; compact?: boolean }) {
  ...
  const trendingBadge = quiz.isTrending ? <Badge className="border-0 bg-[#f59e0b] px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[.1em] text-[#141432] shadow-lg"><Flame size={12} /> Thịnh hành</Badge> : null;
  return (
    <article className="group overflow-hidden rounded-[26px] border border-[#172554]/10 bg-[#fff7e6] shadow-[0_12px_34px_rgba(22,43,62,.055)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_44px_rgba(22,43,62,.11)]">
      {quiz.coverImage ? <div className="relative h-32 overflow-hidden bg-[#ebf4ff]"><img src={quiz.coverImage} alt={`Ảnh bìa ${quiz.title}`} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#141432]/55 via-transparent to-transparent" />{isNew || quiz.isTrending ? <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">{newBadge}{trendingBadge}</div> : null}<div className="absolute bottom-3 left-4 right-4 flex items-end justify-between gap-3"><p className="truncate text-[10px] font-bold uppercase tracking-[.16em] text-white">{quiz.category} · {quiz.lesson}</p><Badge className={isTesting ? "shrink-0 border-0 bg-[#141432] px-2.5 py-1 text-[10px] text-white" : "shrink-0 border-0 bg-white/90 px-2.5 py-1 text-[10px] text-[#141432]"}>{quiz.mode}</Badge></div></div> : null}
      <div className="p-5">
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        {!quiz.coverImage && <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">{newBadge}{trendingBadge}<Badge className={isTesting ? "border-0 bg-[#172554] px-2.5 py-1 text-[10px] text-[#fff7e6]" : "border-0 bg-[#eaf0e6] px-2.5 py-1 text-[10px] text-[#172554]"}>{quiz.mode}</Badge></div>}
      </div>
