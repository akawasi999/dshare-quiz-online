...
export default function QuestionEditorPanel() {
  ...
  return <div className="mx-auto max-w-4xl"><p className="text-[10px] font-bold uppercase tracking-[.17em] text-[#9d7c40]">Dshare / Quản trị / Câu hỏi</p><h1 className="mt-2 font-serif text-[36px] font-semibold tracking-[-.045em] text-[#173a51]">Ngân hàng câu hỏi đa dạng</h1><p className="mt-2 text-sm text-[#6d7d84]">Chọn loại câu hỏi, cấu hình đáp án và để hệ thống kiểm tra các quy tắc riêng trước khi lưu.</p><form onSubmit={submit} className="mt-7 rounded-[26px] border border-[#17334a]/9 bg-white p-5 sm:p-6"><div className="grid gap-4 sm:grid-cols-3"><div><Label>ID Bài học</Label><Input value={lessonId} onChange={event => setLessonId(event.target.value)} required type="number" className="mt-2" /></div><div><Label>ID Bộ đề <span className="font-normal text-[#859197]">(tùy chọn)</span></Label><Input value={quizId} onChange={event => setQuizId(event.target.value)} type="number" className="mt-2" /></div><div><Label>Loại câu hỏi</Label><select value={type} onChange={event => setQuestionType(event.target.value as QuestionValidationType)} className="mt-2 h-10 w-full rounded-md border border-[#17334a]/10 bg-[#f7f7f3] px-3 text-xs"><option value="single">Một đáp án</option><option value="multiple">Nhiều đáp án</option><option value="true_false">Đúng / Sai</option><option value="fill_blank">Điền từ</option><option value="image">Có hình ảnh</option><option value="matching">Ghép nối</option></select></div></div><p className="mt-3 rounded-xl bg-[#f5f1e6] px-3 py-2 text-[11px] leading-5 text-[#735c2d]">{typeDescriptions[type]}</p><div className="mt-5"><Label>Nội dung câu hỏi</Label><Textarea value={prompt} onChange={event => setPrompt(event.target.value)} required className="mt-2 min-h-24" placeholder="Nhập câu hỏi rõ ràng, đủ ngữ cảnh…" /></div>{type === "image" ? <div className="mt-4"><Label>URL hình ảnh minh họa</Label><Input value={imageUrl} onChange={event => setImageUrl(event.target.value)} required type="url" className="mt-2" placeholder="https://…" /></div> : null}{["single", "multiple", "image"].includes(type) ? <OptionEditor type={type} options={options} onUpdate={updateOption} onChooseSingle={chooseSingleCorrect} onToggleMultiple={index => updateOption(index, { isCorrect: !options[index]?.isCorrect })} onAdd={() => setOptions(values => [...values, { body: "", isCorrect: false }])} onRemove={index => setOptions(values => values.length > 2 ? values.filter((_, optionIndex) => optionIndex !== index) : values)} /> : null}{type === "true_false" ? <div className="mt-5"><Label>Đáp án đúng</Label><div className="mt-2 grid grid-cols-2 gap-3">{(["Đúng", "Sai"] as const).map(answer => <button key={answer} type="button" onClick={() => setTrueFalseAnswer(answer)} className={`rounded-xl border px-4 py-3 text-sm font-bold ${trueFalseAnswer === answer ? "border-[#b88f44] bg-[#f8f0da] text-[#604b1e]" : "border-[#17334a]/10 bg-[#fafaf7] text-[#647780]"}`}>{answer}</button>)}</div></div> : null}{type === "fill_blank" ? <div className="mt-5"><Label>Đáp án được chấp nhận</Label><Input value={acceptedAnswers} onChange={event => setAcceptedAnswers(event.target.value)} className="mt-2" placeholder="Ví dụ: JavaScript|JS|Javascript" /><p className="mt-2 text-[11px] text-[#7b898e]">Dùng ký tự <strong>|</strong> để phân tách các cách trả lời hợp lệ.</p></div> : null}{type === "matching" ? <PairEditor pairs={pairs} onChange={setPairs} /> : null}<div className="mt-5"><Label>Lời giải sau khi nộp bài</Label><Textarea value={explanation} onChange={event => setExplanation(event.target.value)} className="mt-2 min-h-20" placeholder="Giải thích vì sao đáp án phù hợp…" /></div><Button disabled={save.isPending} className="mt-6 rounded-full bg-[#173a51]">{save.isPending ? <Loader2 className="animate-spin" size={15} /> : <Save size={15} />} Lưu câu hỏi</Button></form></div>;
}
