...
const typeDescriptions: Record<QuestionValidationType, string> = {
  ...
  essay: "Câu tự luận có dàn ý hoặc đáp án mẫu để chấm thủ công.",
};
...
export default function QuestionEditorPanel() {
  ...
  const answerConfig = useMemo(() => type === "fill_blank" ? { acceptedAnswers: acceptedAnswers.split("|").map(answer => answer.trim()).filter(Boolean) } : type === "matching" ? { pairs: pairs.map(pair => ({ left: pair.left.trim(), right: pair.right.trim() })) } : type === "essay" ? { sampleOutline: sampleOutline.trim() } : {}, [acceptedAnswers, pairs, sampleOutline, type]);
  const submittedOptions = useMemo(() => {
    if (type === "true_false") return [{ body: "Đúng", isCorrect: trueFalseAnswer === "Đúng" }, { body: "Sai", isCorrect: trueFalseAnswer === "Sai" }];
    if (["fill_blank", "matching", "essay"].includes(type)) return [];
    return options.map(option => ({ body: option.body.trim(), isCorrect: option.isCorrect }));
  }, [options, trueFalseAnswer, type]);
