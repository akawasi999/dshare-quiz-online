...
export default function QuizRunner() {
  ...
  const fallback = useMemo(() => isSandbox && sandboxPreview ? {
    id: 0, title: sandboxPreview.title, category: "Studio", subject: "Xem trước", lesson: "Sandbox", summary: sandboxPreview.summary || "Bạn đang xem trước Quiz trong môi trường Sandbox.", mode: "Ôn tập" as const, difficulty: "Tùy chỉnh", duration: `${Math.ceil(sandboxPreview.durationSeconds / 60)} phút`, questionCount: sandboxPreview.questions.length, accent: "#2563eb", points: 0, reward: 0, tier: "Basic",
  } : detail.data ? {
  ...
  } : demoFallback, [detail.data, demoFallback, isSandbox, sandboxPreview]);
  ...
  const begin = async () => {
    if (isSandbox && sandboxPreview) { setAttemptId(-Date.now()); setQuestions(sandboxPreview.questions); setPhase("active"); setTimeLeft(sandboxPreview.durationSeconds); setAnswers({}); setCurrentIndex(0); toast.info("Đang xem trước Sandbox", { description: "Kết quả không được lưu vào lịch sử hoặc thống kê." }); return; }
  ...
  if (phase === "ready" && !isSandbox && detail.isLoading) return <div className="min-h-screen bg-[#fff7e6]"><SiteHeader /><main className="container grid min-h-[70vh] place-items-center"><p role="status" aria-live="polite" className="flex items-center gap-3 text-sm text-[#617786]"><Loader2 className="animate-spin text-[#2563eb]" size={18} />Đang tải thông tin bộ đề…</p></main></div>;
  if (phase === "ready" && !isSandbox && detail.isError) return <div className="min-h-screen bg-[#fff7e6]"><SiteHeader /><main className="container grid min-h-[70vh] place-items-center"><div role="alert" className="max-w-md text-center"><AlertTriangle className="mx-auto text-[#f59e0b]" size={30} /><h1 className="mt-4 font-serif text-3xl font-semibold text-[#172554]">Chưa tải được bộ đề</h1><p className="mt-3 text-sm leading-6 text-[#617786]">{detail.error.message || "Kết nối tạm thời gặp sự cố. Bạn có thể thử tải lại thông tin bộ đề."}</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Button onClick={() => detail.refetch()} className="rounded-full bg-[#172554]">Thử lại</Button><Button asChild variant="outline" className="rounded-full"><Link href="/kham-pha">Quay lại thư viện</Link></Button></div></div></main></div>;
  if (phase === "ready") return <div className="min-h-screen bg-[#fff7e6]"><SiteHeader /><main className="container py-10 lg:py-16"><Link href={isSandbox ? "/tao-quiz" : "/kham-pha"} className="inline-flex items-center gap-2 text-xs font-bold text-[#617786]"><ArrowLeft size={15} /> {isSandbox ? "Quay lại Studio" : "Trở về thư viện"}</Link><section className="mt-8 grid gap-6 lg:grid-cols-[1fr_.65fr]"><div className="rounded-[30px] bg-[#172554] p-7 text-white sm:p-10"><p className="text-[10px] font-bold uppercase tracking-[.17em] text-[#fbbf24]">{isSandbox ? "Preview Sandbox · Không ghi dữ liệu" : `${fallback.category} · ${fallback.subject} · ${fallback.lesson}`}</p><h1 className="mt-4 max-w-2xl font-serif text-[43px] font-semibold leading-[1.05] tracking-[-.05em] sm:text-[57px]">{fallback.title}</h1><p className="mt-6 max-w-xl text-sm leading-6 text-[#eef4ff]">{fallback.summary}</p><div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4"><Stat label="Câu hỏi" value={`${fallback.questionCount}`} /><Stat label="Thời gian" value={fallback.duration} /><Stat label="Ngưỡng đạt" value="70 điểm" /><Stat label="Chế độ" value={isSandbox ? "Sandbox" : fallback.mode} /></div></div><aside className="rounded-[30px] border border-[#172554]/10 bg-white p-7"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#f59e0b]">{isSandbox ? "Xem trước Quiz" : "Xác nhận trước khi bắt đầu"}</p><h2 className="mt-3 font-serif text-[29px] font-semibold leading-tight text-[#172554]">{isSandbox ? "Trải nghiệm Quiz như người học." : "Một lượt làm bài tập trung."}</h2><div className="mt-6 space-y-4 text-xs leading-5 text-[#617786]"><p className="flex gap-3"><ShieldCheck className="shrink-0 text-[#617786]" size={18} />{isSandbox ? "Không tạo attempt, không lưu điểm hoặc thống kê." : "Thứ tự câu hỏi và đáp án được xáo trộn theo từng lượt làm."}</p><p className="flex gap-3"><Clock3 className="shrink-0 text-[#617786]" size={18} />Bài sẽ tự động nộp khi đồng hồ về 00:00.</p></div>{fallback.mode === "Kiểm tra" && <div className="mt-6 rounded-2xl bg-[#fff7e6] p-4 text-xs leading-5 text-[#d97706]"><Sparkles className="mb-2" size={16} />Lệ phí: <strong>{fallback.points} Point</strong>. Đạt từ 70 điểm để nhận <strong>{fallback.reward} Point</strong> thưởng.</div>}<Button onClick={begin} disabled={start.isPending} aria-busy={start.isPending} className="mt-7 h-12 w-full rounded-full bg-[#f59e0b] text-xs font-bold hover:bg-[#d97706]">{start.isPending ? <Loader2 className="animate-spin" size={16} /> : isSandbox ? "Bắt đầu xem trước" : "Bắt đầu làm bài"}</Button></aside></section></main></div>;
