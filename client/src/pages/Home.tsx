...
export default function Home() {
  ...
  const visibleSpotlight = spotlight.filter(quiz => selectedTopic === "Tất cả" || quiz.category === selectedTopic);
  const submitSearch = (event: FormEvent) => {
    event.preventDefault();
    setLocation(`/kham-pha${search.trim() ? `?q=${encodeURIComponent(search.trim())}` : ""}`);
  };
