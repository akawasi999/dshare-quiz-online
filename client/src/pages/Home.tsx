...
import { ArrowRight, BookMarked, BrainCircuit, CheckCircle2, ChevronRight, CircleHelp, Compass, Crown, Layers3, ShieldCheck, Sparkles, TimerReset } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="border-y border-border-light bg-surface py-16 lg:py-24"><div className="container"><div className="flex items-end justify-between gap-5"><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-primary">Bắt đầu từ hôm nay</p><h2 className="mt-3 font-serif font-bold">Bộ đề được quan tâm</h2></div><span className="hidden rounded-full bg-muted px-4 py-2 text-xs font-semibold text-text-secondary md:block">{catalogQuery.isLoading ? "Đang cập nhật" : catalogQuery.isError ? "Chưa thể cập nhật catalog" : selectedTopic === "Tất cả" ? "Mới nhất trong thư viện" : `Chủ đề: ${selectedTopic}`}</span></div>{catalogQuery.isError ? <div role="alert" className="mt-10 flex flex-col items-center rounded-[var(--radius-xl-token)] border border-dashed border-danger/35 bg-danger/5 p-9 text-center"><p className="text-xl font-bold text-foreground">Chưa thể cập nhật thư viện bộ đề.</p><p className="mt-2 max-w-xl text-sm text-text-secondary">Dữ liệu mới nhất tạm thời chưa khả dụng. Bạn có thể thử tải lại danh sách.</p><Button onClick={() => catalogQuery.refetch()} className="mt-5 rounded-full">Thử lại</Button></div> : visibleSpotlight.length ? <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{visibleSpotlight.map(quiz => <QuizCard key={quiz.id} quiz={quiz} />)}</div> : <div className="mt-10 rounded-[var(--radius-xl-token)] border border-dashed border-border bg-muted p-9 text-center"><p className="text-xl font-bold text-foreground">Chưa tìm thấy bộ đề phù hợp.</p><p className="mt-2 text-sm text-text-secondary">Hãy thử từ khóa khác hoặc đặt lại danh mục để xem toàn bộ thư viện.</p><Button onClick={() => { setSearch(""); setSelectedTopic("Tất cả"); }} variant="outline" className="mt-5 rounded-full">Đặt lại bộ lọc</Button></div>}</div></section>
