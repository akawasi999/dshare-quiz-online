...
import { plainTextToRichHtml, sanitizeRichTextHtml } from "@shared/richText";
...
export default function Legal({ document }: { document: LegalDocument }) {
  ...
  const html = sanitizeRichTextHtml(plainTextToRichHtml(raw));
  return <div className="min-h-screen bg-background text-foreground"><SiteHeader /><main className="container py-12 sm:py-16 lg:py-20"><div className="mx-auto max-w-3xl"><p className="text-[11px] font-bold uppercase tracking-[.18em] text-primary">Thông tin pháp lý</p><h1 className="mt-4 font-sans text-4xl font-extrabold tracking-[-.045em] sm:text-5xl">{page.title}</h1><p className="mt-5 max-w-2xl text-base leading-7 text-text-secondary">{page.intro}</p><article className="rich-text-editor-content mt-10 rounded-[var(--radius-lg-token)] border border-border bg-surface p-6 text-sm leading-7 text-text-secondary sm:p-7" dangerouslySetInnerHTML={{ __html: html }} /><p className="mt-7 text-xs text-text-muted">Cập nhật lần cuối: {formatUpdatedAt(updatedAt)}</p></div></main></div>;
}
