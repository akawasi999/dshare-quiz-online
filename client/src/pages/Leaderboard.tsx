...
export default function Leaderboard() {
  ...
  const xpLeaderboard = trpc.leaderboard.xp.useQuery({ period });
          ...
          <div className="border-b border-border-light px-5 py-6 sm:px-7">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-text-secondary">{scope === "all" ? "Xếp hạng theo XP học tập và Level; Point không ảnh hưởng thứ hạng." : "Ưu tiên điểm cao nhất, sau đó là số lượt hoàn thành hợp lệ."}</p>
              </div>
            ...
            {scope === "all" ? <div className="mt-5 inline-flex rounded-full bg-muted p-1" role="group" aria-label="Chọn kỳ xếp hạng XP">{(["all", "week", "month"] as const).map(value => <button key={value} type="button" aria-pressed={period === value} onClick={() => setPeriod(value)} className={`min-h-9 rounded-full px-3 text-[11px] font-bold ${period === value ? "bg-surface text-primary shadow-sm" : "text-text-secondary hover:text-primary"}`}>{value === "all" ? "Tất cả" : value === "week" ? "Tuần này" : "Tháng này"}</button>)}</div> : <div className="mt-6"><label htmlFor="leaderboard-quiz" className="text-[10px] font-bold uppercase tracking-[.13em] text-text-secondary">Chọn bộ đề</label><select id="leaderboard-quiz" value={quizId ?? ""} onChange={event => setQuizId(event.target.value ? Number(event.target.value) : null)} className="mt-2 h-11 w-full rounded-[var(--radius-sm-token)] border border-border bg-background px-3 text-sm text-foreground outline-none transition focus:border-primary focus:ring-4 focus:ring-ring/20" disabled={catalog.isLoading}><option value="">{catalog.isLoading ? "Đang tải bộ đề…" : "Chọn một bộ đề"}</option>{catalog.data?.map(quiz => <option key={quiz.quizId} value={quiz.quizId}>{quiz.title} · {quiz.categoryTitle}</option>)}</select>{catalog.isError ? <div role="alert" className="mt-3 flex flex-wrap items-center gap-3 text-xs text-danger"><p>Chưa tải được danh sách bộ đề. Vui lòng thử lại sau.</p><Button variant="outline" onClick={() => catalog.refetch()} className="h-8 rounded-full px-3 text-[10px]">Thử lại</Button></div> : null}</div>}
          </div>

          <div className="px-5 py-2 sm:px-7">
            {scope === "quiz" && !quizId ? <ScopePrompt /> : scope === "all" ? xpLeaderboard.isLoading ? <div className="flex items-center justify-center gap-2 py-20 text-center text-sm text-text-secondary" role="status"><Loader2 className="animate-spin text-primary" size={17} />Đang cập nhật XP…</div> : xpLeaderboard.isError ? <div className="py-16 text-center" role="alert"><CircleAlertIcon /><h3 className="mt-4 font-serif text-2xl font-semibold text-foreground">Chưa tải được bảng XP</h3><Button onClick={() => xpLeaderboard.refetch()} className="mt-6 rounded-full">Thử lại</Button></div> : xpLeaderboard.data?.length ? <ol className="divide-y divide-border-light">{xpLeaderboard.data.map((entry: any, index: number) => <li key={entry.userId} className="flex items-center gap-4 py-5 first:pt-6"><RankBadge rank={index + 1} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-foreground">{entry.name ?? "Học viên Dshare"}</p><p className="mt-1 text-xs text-text-secondary">{entry.levelName ?? "Beginner"} · chuỗi {entry.currentStreak ?? 0} ngày</p></div><div className="text-right"><p className="font-serif text-2xl font-semibold tracking-[-.03em] text-foreground">{entry.xp.toLocaleString("vi-VN")}<span className="ml-1 text-xs font-medium text-text-secondary">XP</span></p><p className="mt-1 text-[10px] font-bold uppercase tracking-[.12em] text-success">Tiến trình học tập</p></div></li>)}</ol> : <EmptyLeaderboard isQuizScope={false} /> : leaderboard.isLoading ? <div className="flex items-center justify-center gap-2 py-20 text-center text-sm text-text-secondary" role="status"><Loader2 className="animate-spin text-primary" size={17} />Đang cập nhật thành tích…</div> : leaderboard.isError ? <div className="py-16 text-center" role="alert"><CircleAlertIcon /><h3 className="mt-4 font-serif text-2xl font-semibold text-foreground">Chưa tải được bảng thành tích</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-text-secondary">Vui lòng thử lại để xem các dấu mốc mới nhất.</p><Button onClick={() => leaderboard.refetch()} className="mt-6 rounded-full">Thử lại</Button></div> : leaderboard.data?.length ? <ol className="divide-y divide-border-light">{leaderboard.data.map((entry, index) => <li key={entry.userId} className="flex items-center gap-4 py-5 first:pt-6"><RankBadge rank={index + 1} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-foreground">{entry.name ?? "Học viên Dshare"}</p><p className="mt-1 text-xs text-text-secondary">{entry.completedCount} lượt hoàn thành bộ đề này</p></div><div className="text-right"><p className="font-serif text-2xl font-semibold tracking-[-.03em] text-foreground">{entry.bestScore}<span className="ml-1 text-xs font-medium text-text-secondary">đ</span></p><p className="mt-1 text-[10px] font-bold uppercase tracking-[.12em] text-success">Điểm cao nhất</p></div></li>)}</ol> : <EmptyLeaderboard isQuizScope={isQuizScope} />}
          </div>
        </div>

        <aside className="rounded-[var(--radius-xl-token)] border border-border bg-muted p-6 sm:p-7 xl:self-start"><span className="grid size-11 place-items-center rounded-2xl bg-primary-light text-primary"><Crown size={21} /></span><p className="mt-6 text-[10px] font-bold uppercase tracking-[.16em] text-primary">Cách ghi nhận</p><h2 className="mt-2 font-serif text-3xl font-semibold leading-tight tracking-[-.04em] text-foreground">Mỗi lần học đều là một chiến thắng.</h2><div className="mt-7 space-y-5 text-sm leading-6 text-text-secondary"><p><strong className="text-foreground">XP & Level.</strong> Toàn hệ thống xếp hạng theo XP do máy chủ phát từ hoạt động học tập.</p><p><strong className="text-foreground">Point độc lập.</strong> Point là currency premium và không làm thay đổi thứ hạng.</p><p><strong className="text-foreground">Theo Quiz.</strong> Bạn vẫn có thể chuyển sang bảng điểm cao nhất cho từng bộ đề.</p></div><div className="mt-7 flex items-center gap-2 rounded-[var(--radius-md-token)] border border-primary/10 bg-surface px-4 py-3 text-xs font-semibold text-primary"><Sparkles size={15} />Tiếp tục học để tạo dấu mốc mới.</div></aside>
      </section>
    </main>
  </div>;
}
