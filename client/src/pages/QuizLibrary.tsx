...
export default function QuizLibrary() {
  ...
  return <div className="min-h-screen bg-[#fffdf8]"><SiteHeader />
    <main>
      ...
      <section className="container py-10 lg:py-14"><div className="flex flex-col gap-5 rounded-[24px] border border-[#17334a]/9 bg-white p-4 shadow-sm lg:flex-row lg:items-center"><div className="flex h-12 flex-1 items-center rounded-xl bg-[#f2f3ef] px-4"><Search size={17} className="text-[#9b7c41]" /><input value={search} onChange={event => setSearch(event.target.value)} className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-[#99a2a5]" placeholder="Tên bộ đề, môn học hoặc bài học" /></div><div className="flex items-center gap-2 overflow-x-auto"><Filter size={16} className="shrink-0 text-[#68747b]" />{filters.map(item => <button key={item} onClick={() => setCategory(item)} className={cn("shrink-0 rounded-full px-3 py-2 text-xs font-semibold", category === item ? "bg-[#183a51] text-white" : "bg-[#f2f3ef] text-[#526876] hover:bg-[#e6e8e3]")}>{item}</button>)}</div></div>
        <div className="mt-9 flex flex-col justify-between gap-4 border-b border-[#17334a]/9 pb-5 sm:flex-row sm:items-center"><p className="text-sm font-semibold text-[#405765]"><span className="font-serif text-[24px] text-[#183a51]">{filtered.length}</span> bộ đề phù hợp với bạn</p><div className="flex items-center gap-2"><SlidersHorizontal size={15} className="text-[#8b754c]" />{modes.map(item => <button key={item} onClick={() => setMode(item)} className={cn("rounded-full px-3 py-2 text-xs font-semibold", mode === item ? "bg-[#e8d9b0] text-[#5b451f]" : "text-[#6c7780] hover:bg-[#f2f3ef]")}>{item}</button>)}</div></div>
