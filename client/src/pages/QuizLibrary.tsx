...
export default function QuizLibrary() {
  ...
  useEffect(() => {
    window.localStorage.setItem(libraryPreferenceKey, JSON.stringify({ category, difficulty } satisfies LibraryPreferences));
  }, [category, difficulty]);
  ...
  useEffect(() => {
    ...
    return () => { list.removeEventListener("scroll", updateScrollState); window.removeEventListener("resize", updateScrollState); };
  ...
  return <div className="min-h-screen bg-[#fff7e6]"><SiteHeader />
    <section className="bg-[#fff7e6]"><div className="container pb-3 pt-5 lg:pb-4 lg:pt-6"><div className="rounded-[22px] border border-[#007453]/20 bg-[#e8f6fd] px-5 py-4 sm:flex sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#007453] text-white"><Sparkles size={19} /></span><div><p className="text-sm font-bold text-[#141432]">{hasUnlimitedAttempts ? "Bạn có lượt làm Quiz không giới hạn" : `Bạn còn ${quota.data?.remaining.attempts ?? "—"} lượt làm Quiz tháng này`}</p><p className="mt-1 text-xs text-[#6c6c7a]">Theo gói {quota.data?.tier?.toUpperCase() ?? "BASIC"}.</p></div></div><div className="mt-3 flex w-full items-end gap-3 sm:mt-0 sm:w-auto"><div className="min-w-0 flex-1 sm:w-44 sm:flex-none"><div className="mb-1.5 flex items-center justify-end text-[10px] font-bold uppercase tracking-[.1em] text-[#007453]"><span>{hasUnlimitedAttempts ? "∞" : `${usedAttempts}/${attemptLimit ?? 0}`}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/85" role="progressbar" aria-label="Tiến độ lượt làm Quiz" aria-valuemin={0} aria-valuemax={hasUnlimitedAttempts ? undefined : attemptLimit ?? 0} aria-valuenow={hasUnlimitedAttempts ? undefined : usedAttempts}><div className="h-full rounded-full bg-[#007453] transition-[width] duration-300" style={{ width: `${hasUnlimitedAttempts ? 100 : quotaProgress}%` }} /></div></div>{shouldShowUpgrade ? <Button asChild size="sm" className="shrink-0 rounded-full bg-[#065be5] px-4 text-xs font-bold text-white hover:bg-[#065be5]"><Link href="/bang-gia">Nâng cấp ngay <ArrowRight size={14} /></Link></Button> : null}</div></div></div></section>
    <section className="container pb-10 pt-5 lg:pb-14 lg:pt-6">
      ...
      <div className="rounded-[24px] border border-[#172554]/9 bg-white p-3 shadow-sm"><div className="flex min-w-0 items-center gap-3"><div className="flex shrink-0 items-center gap-1.5 pl-1 text-[10px] font-bold uppercase tracking-[.12em] text-[#617786]"><Filter aria-hidden="true" size={14} /> Chủ đề</div><div className="relative min-w-0 flex-1"><div ref={categoryListRef} className="-mr-1 flex min-w-0 snap-x snap-mandatory gap-2 overflow-x-auto scroll-smooth pb-1 pr-1 [scrollbar-width:thin]">{categoryFilters.map((item: string) => { const Icon = getCategoryIcon(item); return <button key={item} aria-pressed={category === item} onClick={() => { setHasStartedExploring(true); setCategory(item); }} className={cn("flex shrink-0 snap-start items-center gap-1.5 rounded-full px-3 py-2 text-xs font-semibold", category === item ? "bg-[#172554] text-white" : "bg-[#eef4ff] text-[#617786] hover:bg-[#dbeafe]")}><span className={cn("grid h-5 w-5 place-items-center rounded-full", category === item ? "bg-white/20" : "bg-white text-[#065be5]")}><Icon size={12} /></span>{item}<span className={cn("rounded-full px-1.5 py-0.5 text-[10px] font-bold", category === item ? "bg-white/20 text-white" : "bg-white text-[#065be5]")}>{categoryCounts[item] ?? 0}</span></button>; })}</div>{canScrollLeft ? <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-0 hidden w-10 bg-gradient-to-r from-white via-white/85 to-transparent sm:block" /> : null}{canScrollRight ? <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-0 hidden w-12 bg-gradient-to-l from-white via-white/85 to-transparent sm:block" /> : null}</div><div className="hidden shrink-0 items-center gap-1 lg:flex"><button type="button" aria-label="Cuộn chủ đề sang trái" disabled={!canScrollLeft} onClick={() => scrollCategories("left")} className="grid h-8 w-8 place-items-center rounded-full border border-[#172554]/10 text-[#172554] transition hover:bg-[#eef4ff] disabled:cursor-not-allowed disabled:opacity-35"><ArrowLeft size={15} /></button><button type="button" aria-label="Cuộn chủ đề sang phải" disabled={!canScrollRight} onClick={() => scrollCategories("right")} className="grid h-8 w-8 place-items-center rounded-full border border-[#172554]/10 text-[#172554] transition hover:bg-[#eef4ff] disabled:cursor-not-allowed disabled:opacity-35"><ArrowRight size={15} /></button></div></div></div>
      <div className="mt-7 flex justify-center border-b border-[#172554]/9 pb-5"><div className="flex flex-wrap justify-center gap-2">{["Tất cả", "Dễ", "Trung bình", "Nâng cao"].map(item => <button key={item} aria-pressed={difficulty === item} onClick={() => { setHasStartedExploring(true); setDifficulty(item); }} className={cn("rounded-full border px-4 py-2 text-xs font-bold", difficulty === item ? "border-[#3762d2] bg-[#3762d2] text-white" : "border-[#172554]/10 bg-white text-[#6c6c7a]")}>{item === "Tất cả" ? <Target className="mr-1 inline" size={13} /> : <i className={cn("mr-1 inline-block h-2.5 w-2.5 rounded-full", item === "Dễ" ? "bg-[#007453]" : item === "Trung bình" ? "bg-[#f59e0b]" : "bg-[#de1264]")} />} {item}</button>)}</div></div>
