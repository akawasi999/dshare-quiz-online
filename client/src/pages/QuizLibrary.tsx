...
type LibraryPreferences = { search: string; category: string; mode: string; difficulty: string; tier: string; onlyNew: boolean; sort: "newest" | "attempts" | "reward" };
const defaultLibraryPreferences: LibraryPreferences = { search: "", category: "Tất cả", mode: "Tất cả", difficulty: "Tất cả", tier: "Tất cả", onlyNew: false, sort: "newest" };
...
export default function QuizLibrary() {
  ...
  const [search, setSearch] = useState(initialSearch || savedPreferences.search);
  ...
  const [tier, setTier] = useState(savedPreferences.tier);
  const [onlyNew, setOnlyNew] = useState(savedPreferences.onlyNew);
  ...
  const filtered = useMemo(() => liveQuizzes.filter(quiz => {
    ...
    return (!search || haystack.includes(search.toLowerCase())) && (category === "Tất cả" || quiz.category === category) && (mode === "Tất cả" || quiz.mode === mode) && (difficulty === "Tất cả" || quiz.difficulty === difficulty) && (tier === "Tất cả" || quiz.tier === tier) && (!onlyNew || isQuizNew(quiz.createdAt));
  }), [liveQuizzes, search, category, mode, difficulty, tier, onlyNew]);
  ...
  useEffect(() => { window.localStorage.setItem(libraryPreferenceKey, JSON.stringify({ search, category, mode, difficulty, tier, onlyNew, sort } satisfies LibraryPreferences)); }, [search, category, mode, difficulty, tier, onlyNew, sort]);
  useEffect(() => { setVisibleCount(6); }, [search, category, mode, difficulty, tier, onlyNew, sort]);
