...
export default function QuizLibrary() {
  ...
  return <div className="min-h-screen bg-[#fff7e6]"><SiteHeader />
    <main>
      <section className="border-b border-[#172554]/8 bg-[#ebf4ff]"><div className="container py-8 lg:py-10"><div className="rounded-[22px] border border-[#007453]/20 bg-[#e8f6fd] px-5 py-4 sm:flex sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#007453] text-white"><Sparkles size={19} /></span><div><p className="text-sm font-bold text-[#141432]">Bạn còn lượt trải nghiệm Quiz miễn phí</p><p className="mt-1 text-xs text-[#6c6c7a]">Làm quiz để theo dõi tiến độ và nhận gợi ý học tập phù hợp.</p></div></div><div className="mt-3 flex items-center gap-2 text-xs font-bold text-[#007453] sm:mt-0"><span className="flex gap-1"><i className="h-3 w-3 rounded-full bg-[#007453]" /><i className="h-3 w-3 rounded-full bg-[#007453]" /><i className="h-3 w-3 rounded-full bg-[#007453]" /></span> 3 / 5</div></div></div></section>
      <section className="container py-10 lg:py-14">{suggestedCategory && !hasStartedExploring ? <div className="mb-5 flex items-center gap-3 rounded-2xl border border-[#fbbf24] bg-[#fff7e6] px-4 py-3 text-xs leading-5 text-[#d97706]"><Sparkles size={16} className="shrink-0 text-[#f59e0b]" /><span><strong>Gợi ý từ phiên luyện tập gần nhất:</strong> {suggestedCategory.title}</span></div> : null}<div className="flex flex-col gap-5 rounded-[24px] border border-[#172554]/9 bg-white p-4 shadow-sm lg:flex-row lg:items-center"><div className="flex h-12 flex-1 items-center rounded-xl bg-[#eef4ff] px-4"><Search aria-hidden="true" size={17} className="text-[#f59e0b]" /><input aria-label="Tìm kiếm bộ đề" value={search} onChange={event => { setHasStartedExploring(true); setSearch(event.target.value); }} className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-[#617786]" placeholder="Tên bộ đề, môn học hoặc bài học" /></div><div className="flex items-center gap-2 overflow-x-auto"><Filter aria-hidden="true" size={16} className="shrink-0 text-[#617786]" />{categoryFilters.map((item: string) => <button key={item} aria-pressed={category === item} onClick={() => { setHasStartedExploring(true); setCategory(item); }} className={cn("shrink-0 rounded-full px-3 py-2 text-xs font-semibold", category === item ? "bg-[#172554] text-white" : "bg-[#eef4ff] text-[#617786] hover:bg-[#eef4ff]")}>{item}</button>)}</div></div>
