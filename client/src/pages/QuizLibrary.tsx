...
type LibraryPreferences = { search: string; category: string; mode: string; difficulty: string; tier: string; sort: "newest" | "attempts" | "reward" };
const defaultLibraryPreferences: LibraryPreferences = { search: "", category: "Tất cả", mode: "Tất cả", difficulty: "Tất cả", tier: "Tất cả", sort: "newest" };
...
export default function QuizLibrary() {
  ...
  const [hasStartedExploring, setHasStartedExploring] = useState(Boolean(initialSearch || savedPreferences.search || savedPreferences.category !== "Tất cả" || savedPreferences.difficulty !== "Tất cả" || savedPreferences.tier !== "Tất cả"));
  ...
  const filtered = useMemo(() => liveQuizzes.filter(quiz => {
    ...
    return (!search || haystack.includes(search.toLowerCase())) && (category === "Tất cả" || quiz.category === category) && (mode === "Tất cả" || quiz.mode === mode) && (difficulty === "Tất cả" || quiz.difficulty === difficulty) && (tier === "Tất cả" || quiz.tier === tier);
  }), [liveQuizzes, search, category, mode, difficulty, tier]);
  ...
  useEffect(() => { window.localStorage.setItem(libraryPreferenceKey, JSON.stringify({ search, category, mode, difficulty, tier, sort } satisfies LibraryPreferences)); }, [search, category, mode, difficulty, tier, sort]);
  useEffect(() => { setVisibleCount(6); }, [search, category, mode, difficulty, tier, sort]);
