...
export default function Practice() {
  ...
  if (practice.isLoading) return <PracticeShell><p className="py-20 text-sm text-[#617786]">Đang chuẩn bị các câu cần ôn…</p></PracticeShell>;
...
function FeedbackCard({ correct, explanation, correctAnswer, hidden = false }: { correct: boolean; explanation: string | null; correctAnswer: string; hidden?: boolean }) {
  ...
  return <div className={`mt-6 rounded-2xl p-4 text-sm leading-6 ${correct ? "bg-[#edf5eb] text-[#3b653e]" : "bg-[#f8eae6] text-[#617786]"}`}>{correct ? <CheckCircle2 className="mb-2" size={18} /> : <CircleAlert className="mb-2" size={18} />}<strong>{correct ? "Chính xác." : "Chưa đúng."}</strong>{!correct && correctAnswer ? <p className="mt-1">Đáp án đúng: <strong>{correctAnswer}</strong></p> : null}<p className="mt-1">{explanation ?? "Hãy đọc lại kiến thức liên quan và thử diễn giải đáp án đúng theo cách của bạn."}</p></div>;
}
