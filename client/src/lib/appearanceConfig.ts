export type AppearanceConfig = {
  ...
  footer: { enabled: boolean; height: number; width: "container" | "full"; shadow: boolean; border: boolean; columns: 1 | 2 | 3 | 4; description: string; copyright: string; showThemeSwitcher: boolean; links: Array<{ label: string; url: string; enabled: boolean; icon?: FooterLinkIcon }>; linkGroups: Array<{ id: string; title: string; links: Array<{ label: string; url: string; enabled: boolean; icon?: FooterLinkIcon }> }>; socialLinks: Array<{ platform: FooterSocialPlatform; url: string; enabled: boolean; zone?: FooterSocialZone }>; socialStyle: { shape: "round" | "square"; colorMode: "platform" | "custom"; backgroundColor: string; iconColor: string; size: number; showOnMobile: boolean } };
...
export type FooterSocialZone = "brand" | "navigation" | "bottom";
...
export function mergeAppearanceConfig(value: unknown): AppearanceConfig {
  ...
  return {
    ...
    footer: { ...mergedFooter, linkGroups: candidateFooter && !Array.isArray(candidateFooter.linkGroups) && legacyFooterLinks ? [{ id: "information", title: "Thông tin", links: legacyFooterLinks }] : mergedFooter.linkGroups, socialLinks: Array.isArray(candidateFooter?.socialLinks) ? candidateFooter.socialLinks.map(link => ({ ...link, zone: link.zone ?? "brand" })) : defaultAppearanceConfig.footer.socialLinks, socialStyle: { ...defaultAppearanceConfig.footer.socialStyle, ...candidateFooter?.socialStyle } },
